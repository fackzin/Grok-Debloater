package com.aistudio.shizukuappmanager.shizuku
...
object ShizukuManager {
    ...
    /**
     * Reinstala/restaura um aplicativo via Shizuku.
     * Se for do sistema (apkPath == "system"), usa "pm install-existing".
     * Se houver APK físico persistido, executa "pm install -r".
     */
    suspend fun restoreApp(packageName: String, apkPath: String): Pair<Boolean, String> = withContext(Dispatchers.IO) {
        if (!isActive()) {
            return@withContext Pair(false, "Serviço Shizuku inativo.")
        }
        if (!isPermissionGranted()) {
        ...
        try {
            val command = if (apkPath == "system") {
                arrayOf("pm", "install-existing", packageName)
            } else {
                arrayOf("pm", "install", "-r", apkPath)
            }

            Log.d(TAG, "Iniciando restauração via Shizuku: $packageName")
            val method = Shizuku::class.java.getDeclaredMethod(
                "newProcess",
                Array<String>::class.java,
                Array<String>::class.java,
            ...
            val isSuccess = exitCode == 0 || stdoutStr.contains("Success", ignoreCase = true) || stdoutStr.contains("Installed", ignoreCase = true)
            val resultMsg = if (isSuccess) {
                "Sucesso: $stdoutStr"
            } else {
                "Falha ($exitCode): ${stderrStr.ifEmpty { stdoutStr.ifEmpty { "Erro desconhecido" } }}"
            ...
            Log.e(TAG, "Erro ao restaurar o app $packageName", e)
            Pair(false, e.localizedMessage ?: "Erro de E/S ou de execução Binder")
        }
    }
}
